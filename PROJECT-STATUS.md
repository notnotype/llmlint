...
- 三个正交维度：`level`（high/medium/low，严重度+退出码）、`review`（agent/human/none，审查受众，`check` 默认 `--review agent`）、`fixability`（auto/candidate/manual，机械修复能力）。默认 review 桶为 agent=132 / human=165 / none=3；默认 regex 规则为 auto=3、candidate=0、manual=276；`action.replace` 只是替换模板，不授予应用权限。
...
- 默认规则 materialize 结果（Task 23 规则整理后）：360 total / 300 active；279 regex / 8 density / 5 handler / 8 LLM；review = agent 132 / human 165 / none 3；regex fixability = `auto=3 / candidate=0 / manual=276`。用户配置仍可把指定 regex replace 提升为 candidate。
