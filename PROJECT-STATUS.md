...
llmlint 的核心资产是一个**中文正文规则库**，有两个消费时机：**写作期** `guide` 把规则投影成动笔前的写作约束（markdown，可注入系统提示词或存成文风预设，不需要输入文件）；**审查期** `check` / `fix` / `detect` 用正则确定性定位「AI 味」候选，`rules --detector semantic` 交出静态查不到的语义规则，Agent Skill 读上下文判断并在用户审批后改写。当前版本 **2.0.1**（真相源 = `skill/package.json.version`），已从 neuro-book 内嵌 skill 拆分为独立开发仓（真相源 = 本仓，`github.com/notnotype/llmlint`）。

**写作期证据状态（2026-07-27，guide-arm 三模型面板 + delivery-arm 之后）**：**收益必须逐模型实测，推不出来——同一份约束在不同模型上是完全不同的画像。**
...
- **Opus 5 上未定**（`delivery-arm`，15 对 × 3 臂）：约束进 system prompt 确实提高遵守度（注入规则命中对 tool result 臂 1/15、p = 0.001）、规则侧朝人类基线移动（docScore 5.349 → 3.462，p = 0.035，人类 2.47），但**篇幅显著缩短 24%**（13/15、p = 0.007）。主指标 9/15、p = 0.607 **不能读成无效**——Opus 5 的 docPAi 基线 0.227 已低于人类 reference 0.285，指标没有下降空间。
...
- **投递位置有影响但不是主因**：system prompt 明显优于 tool result（后者是 Agent 宿主里的实际投递方式）。
- **合并面板数字会撒谎**：78 对合并跑五行里四行显著，掩盖「质量信号全来自 deepseek、字数信号主要来自 gemini」。跨模型结论必须先 `guide-compare --model` 分层看。

所以写作期是「deepseek 有证据、gemini 不建议、mimo 与 claude 系未定、整体未验收」。**D5 第二条件的缺口现在是硬阻塞**——gemini 与 Opus 5 都撞上篇幅显著缩短，人评 `wantReadOn` 拿不到就无法判断「写短了」是否同时「变差了」；它同时也是 claude 系唯一现成的独立裁判（规则侧是循环指标）。产品上不要无条件推荐注入，也不要当成已确认有效的功能对外表述。
...
- [CONTEXT.md](CONTEXT.md)：领域语言（术语）+ 硬不变量 I1–I25。
- [evals/METHODOLOGY.md](evals/METHODOLOGY.md)：评测方法论 / 流程规范（代码遵守它，冲突以它为准）。

## Architecture

- 仓库根是开发工作区（name=`llmlint-dev`），承载 `skill/`、`evals/`、`tests/` 和开发脚本。
- `skill/` 是可安装、可发布的 runtime 包（name=`llmlint`）：`bin/llmlint.ts` + `src/`（config / rules / scanner / reporter / guide / markdown-mask / cli）+ `rulesets/builtin/default/` + `references/` + `SKILL.md`。CLI 命令面 = `guide`（写作期）+ `check` / `fix` / `detect` / `rules`（审查期）+ `status` / `config`。
...
- 规则模型 v3 已落地：`scope.layer` 支持 narrative/dialogue/all 等长视图，`ignoreTerms` 统一遮罩 regex/density/handler，density detector 负责全文/段落分布指纹，builtin handler 负责 not-is 状态机、碎句号、过度精炼、低连接密度、引号强调和单段过长；CLI、web 本地扫描、服务端 MachineScan 与 Agent `lint_check` 均消费同一份预烘静态规则。未知 detector/handler 按 warning 优雅跳过。
...
- `check --format json` 默认输出**紧凑形态**：规则元数据按 id 去重到顶层 `rules`、命中只带 `ruleId`、`context` 各裁 24 码点、`registry` 去掉逐 namespace 明细、不缩进。投影在 `skill/src/check-report.ts`（纯函数、无 picocolors，CLI 与 web「复制 JSON」共用）。`--rule-detail` 恢复完整内联形态供写规则/排查 overlap 用。实测同一样本 `--review all` 从 84936 降到 19720 字节（−77%；随后规则 `note` 变长使绝对值上移到 20856 / detail 88802，压缩比不变）。
...
- `status` / `config` 提供用户级 `~/.llmlint/settings.json` 初始化门；`detect` 直连 HF Space `yuchuantian-aigc-text-detector.hf.space`，句界分块、代理分流、content-hash 缓存。检测结论分两层：整篇用绝对阈值 `docPAi >= 0.85`，文内一律用相对排序（`rank` / `relative` / `spread` 为报告层派生字段，不进缓存），`spread < 0.15` 时报告「整篇均匀」并声明四象限不适用——绝对阈值在整体 AI 文本上会把全文标红。`spread` 落在门槛 ±0.05 的边界带时 stylish 额外声明「位次是弱证据」；0.15 与 0.05 都是未校准起点，只有两篇样本支撑，提示词侧已写明。
- Agent Skill 使用“首次 install 依赖门 + 五步闭环”：当前安装首次启用、更新后或 `node_modules` 缺失时，先在 skill 根执行 `bun install --frozen-lockfile`，成功后才进入 `status`（该门是软门，`initialized:false` 不阻塞 `check`/`detect`）。修复提示词把静态命中与文内位次视为候选证据，先按修/留/问分流，再按删/压/换做最小改动；不以清零命中或降低检测分数为目标。**创作类正文默认跑 `--review all`**——规则整理把大量语境敏感规则下沉到 human 桶，只看 agent 桶会漏掉主要问题；`agent` 桶是可修入口，`human` 桶只进「问 / 留」。复测判据是静态命中减少且无新命中，检测分数只作参考（实测一轮修复后 docPAi 可能反升）。
...
| Task | Status | Notes |
| --- | --- | --- |
| [24 多轮修订谱系与数据收集通道](docs/tasks/24-revision-rounds-and-contribute/README.md) | Designed | 2026-07-27 设计定稿 + 同日审查修正（Task 23 分片 2 演化）：**收集本地优先**——`contribute` 按四档裁剪落用户级发件箱 `~/.llmlint/outbox/`，web 端点与发送后置到服务轮；新增**多轮修订谱系** `.agent/llmlint/rounds/NNNN/{source/,output/,plan.md}`（作废 polish 单槽拍板，full 档任何轮可用）；台账升 v3（`projectId` 匿名分组键 + 修前修后 `wantReadOn` 人评 + 显式 `parentRound`）；信封含 `kind` 扩展位（记忆系统下任务接入）/`contentHash`/`sourceHash`（stats 档零原文仍可跨轮关联）；隐私裁剪走白名单构造 + 哨兵不变量入测试。**contribute 在步骤 5 自动执行**（复用 `sharing.mode`，默认翻成 `auto`，consent 落点是初始化门；`--auto` 由 CLI 读 settings 决定落/跳过/待确认）。**调研发现合规红线已被突破**（public 历史含 26 篇版权章节全文），Phase 0 = filter-repo 清 `evals/corpus` 全历史 + 一次性授权 force push（备份必须在仓外，分支备份会被一并重写）。盲评种子：guide-arm 156 篇导入本地 web（arm 不得出现在任何发往客户端的字段，含 `provenanceJson`）供 wantReadOn 盲评。审查 10 条修正已写回 README。实施未开始。 |
...
| [02 llmlint Rule Registry](docs/tasks/02-llmlint-rule-registry/README.md) | Implemented | flat Rule Registry（id/namespace/ruleset）、三层覆盖、review/fixability 维度、rules 目录递归加载、CLI `fix`+多文件+Markdown 遮罩；Task 23 规则整理后默认 auto=2/candidate=0，并以版本化 creative profile 收敛 8 条稳定重复规则，不删除全局规则资产。 |
...
- **语料合规**：⚠ **红线已被突破**（2026-07-27 发现）——仓库已 public（07-16 前已推送）且历史含 `evals/corpus/` 全部 162 文件、其中 28 篇为受版权章节全文。处置已拍板：git filter-repo 清全历史 + 一次性授权 force push，本地保留 + gitignore（[Task 24](docs/tasks/24-revision-rounds-and-contribute/README.md) Phase 0，未执行）。
