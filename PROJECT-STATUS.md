...
**写作期证据状态（2026-07-27，两轮实验后）**：**收益取决于模型的检测器基线；claude 系上主指标失去分辨力，结论未定。**

- **deepseek-v4-flash 上有效**（`guide-arm`，26 对）：主指标 docPAi 0.896 → 0.776、留出规则命中同步下降，均 20/26、p = 0.009；注入规则命中不显著这一点排除了「表层规避」这个替代解释。
...
- **关键分辨力事实**（`externalDetector.byModel`）：gemini 0.969 / deepseek 0.945 / gpt-5.5 0.941 / mimo 0.923，而 claude-opus-4-8 **0.130** / claude-fable-5 **0.071**，人类 **0.285**。**主指标只在基线明显高于人类的模型上可用**；测 claude 系必须换裁判。
...
所以写作期是「基线高的模型有证据、claude 系待换指标重测、整体未验收」。**D5 第二条件的缺口现在是硬阻塞**——`delivery-arm` 恰好撞上篇幅缩短，人评 `wantReadOn` 拿不到就无法判断「写短了」是否同时「变差了」；而它同时也是 claude 系唯一现成的独立裁判（规则侧是循环指标）。产品上不要无条件推荐注入，也不要当成已确认有效的功能对外表述。
