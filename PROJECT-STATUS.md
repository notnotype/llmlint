...
- web 的 LLM 分析/改写现依赖深 Module `AgentHarnessPort`；当前 Adapter=`LocalAgentHarness`（Prisma append-only session/entry/invocation + NeuroBook 风格 SSE），语义对齐 `NeuroAgentHarness` 黑盒合同。SSE 已具备 `eventEpoch + seq` 游标、500 条 replay、snapshotRequired 恢复以及 Pi text/thinking/tool/turn 原生流事件；未来抽成独立包时只替换 Adapter，不改数据库/API/前端状态机。
