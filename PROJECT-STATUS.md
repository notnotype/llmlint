...
- `check --format json` 默认输出**紧凑形态**：规则元数据按 id 去重到顶层 `rules`、命中只带 `ruleId`、`context` 各裁 24 码点、`registry` 去掉逐 namespace 明细、不缩进。投影在 `skill/src/check-report.ts`（纯函数、无 picocolors，CLI 与 web「复制 JSON」共用）。`--rule-detail` 恢复完整内联形态供写规则/排查 overlap 用。实测同一样本 `--review all` 从 84936 降到 19720 字节。
