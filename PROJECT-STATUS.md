...
- `status` / `config` 提供用户级 `~/.llmlint/settings.json` 初始化门；`detect` 直连 HF Space `yuchuantian-aigc-text-detector.hf.space`，句界分块、代理分流、content-hash 缓存。检测结论分两层：整篇用绝对阈值 `docPAi >= 0.85`，文内一律用相对排序（`rank` / `relative` / `spread` 为报告层派生字段，不进缓存），`spread < 0.15` 时报告「整篇均匀」并声明四象限不适用——绝对阈值在整体 AI 文本上会把全文标红。
