...
- `skill/` 已是 v2.0.0 自包含包：`check`（regex 候选定位 + markdown 遮罩）/ `fix`（仅 auto 桶）/ `show-llm-rules`；当前默认规则为 360 rules / 313 active；`llmlint.config.ts` 三层覆盖（rule id > namespace > ruleset）。
...
- 默认规则整理（二批）：继续默认关闭旧 `not-but-structure` / `not-x-is-y` / `negative-listing` 与两个无效星号二元规则，二元对比统一交给 story-deslop handler / 校准规则；默认关闭对白冒号替换、宽泛 simile-like、重复 body target 和过宽“规律”。
...
- 当前默认 materialize：360 total / 313 active；292 regex / 8 density / 5 handler / 8 LLM；review = agent 152 / human 158 / none 3；regex fixability = auto 3 / candidate 0 / manual 289。
