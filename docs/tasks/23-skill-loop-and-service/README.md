...
- 默认规则整理（八批）：默认关闭素材通配符转换遗留：4 条嘴角规则和 `cn.tone.tone-placeholder` 里的 `*` 被转换为字面量星号，实际只会命中带星号文本；保留资产等待重新建模。裸词级 `拆解`、`甚至是`、`因为惯性`、`外壳` 转 `human`，不再打扰默认 Agent。
...
- 当前默认 materialize：360 total / 295 active；274 regex / 8 density / 5 handler / 8 LLM；review = agent 105 / human 187 / none 3；regex fixability = auto 3 / candidate 0 / manual 271。临时内存复算当前语料：overlap duplicate rate ≈2.4%，human Agent false rate ≈0.39/千字（未改写 `evals/report/report.json`）。
