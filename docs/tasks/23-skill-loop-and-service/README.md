...
- 当前默认 materialize：360 total / 284 active；263 regex / 8 density / 5 handler / 8 LLM；review = agent 57 / human 225 / none 2；regex fixability = auto 2 / candidate 0 / manual 261。临时内存复算当前语料：active regex raw hits 1118；active 同 span overlap = 0、overlap duplicate rate = 0；默认 Agent 桶 reference 侧仍只剩 `vague-amount-noun` 2 处和 `repeated-de-pairs` 1 处少量强判别权衡（未改写 `evals/report/report.json`）。
