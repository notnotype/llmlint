...
- 当前默认 materialize：360 total / 287 active；266 regex / 8 density / 5 handler / 8 LLM；review = agent 82 / human 202 / none 3；regex fixability = auto 3 / candidate 0 / manual 263。临时内存复算当前语料：active 同 span overlap = 0、overlap duplicate rate = 0；默认 Agent 桶 reference 侧仍只剩 `vague-amount-noun` 2 处和 `repeated-de-pairs` 1 处少量强判别权衡（未改写 `evals/report/report.json`）。
