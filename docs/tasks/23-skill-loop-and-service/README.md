...
- `skill/` 已是 v2.0.0 自包含包：`check`（regex 候选定位 + markdown 遮罩）/ `fix`（仅 auto 桶）/ `show-llm-rules`；当前默认规则为 360 rules / 312 active；`llmlint.config.ts` 三层覆盖（rule id > namespace > ruleset）。
...
- 当前默认 materialize：360 total / 312 active；291 regex / 8 density / 5 handler / 8 LLM；review = agent 150 / human 159 / none 3；regex fixability = auto 3 / candidate 0 / manual 288。
